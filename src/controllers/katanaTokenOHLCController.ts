// src/controllers/katanaTokenOHLCController.ts
...
// Cache configuration
const TOKEN_OHLC_CACHE_PREFIX = "token_ohlc_katana_";
...
function getTimeRange(days: number, timeframe: "hour" | "day") {
  ...
  console.log(
    `[Katana Token Debug] Current timestamp: ${endTime} (${new Date(
      endTime * 1000
    ).toISOString()})`
  );
  console.log(`[Katana Token Debug] Calculating time range for ${days} days:`, {
    endTime,
    startTime,
  ...
  if (timeframe === "day") {
    ...
    console.log(`[Katana Token Debug] Day timeframe - converted to days:`, {
  ...
  console.log(`[Katana Token Debug] Hour timeframe - using unix timestamps:`, {
    startTime,
    endTime,
  });
...
/**
 * Get token OHLC data from Katana SushiSwap subgraph
 */
export async function getKatanaTokenOHLCData(
  req: Request<
    {},
    {},
    {},
  ...
  try {
    ...
    console.log(`[Katana Token Debug] Request received with params:`, {
      tokenAddress,
      resolution,
      days,
      force,
    ...
    // Validate input
    if (!tokenAddress) {
      console.log(`[Katana Token Debug] Missing tokenAddress parameter`);
      res.status(400).json({
        status: "error",
    ...
    if (!/^0x[a-f0-9]{40}$/i.test(tokenAddress)) {
      console.log(
        `[Katana Token Debug] Invalid address format: ${tokenAddress}`
      );
      res.status(422).json({
        status: "error",
    ...
    if (!validResolutions.includes(resolution)) {
      console.log(`[Katana Token Debug] Invalid resolution: ${resolution}`);
      res.status(400).json({
        status: "error",
    ...
    if (isNaN(daysNum) || daysNum < 1 || daysNum > 365) {
      console.log(`[Katana Token Debug] Invalid days parameter: ${days}`);
      res.status(400).json({
        status: "error",
    ...
    console.log(
      `[Katana Token Debug] Normalized address: ${normalizedAddress}`
    );
    console.log(`[Katana Token Debug] Cache key: ${cacheKey}`);

    // Check cache unless force refresh
    if (!force) {
      console.log(`[Katana Token Debug] Checking cache for key: ${cacheKey}`);
      try {
        const cached = await getValue(cacheKey);
        if (cached) {
          console.log(
            `[Katana Token Debug] Cache hit for token OHLC: ${normalizedAddress}`
          );
          ...
          console.log(`[Katana Token Debug] Cache miss for key: ${cacheKey}`);
        }
      } catch (cacheError) {
        console.warn(
          `[Katana Token Debug] Cache read error for ${cacheKey}:`,
          cacheError
        );
      }
    } else {
      console.log(
        `[Katana Token Debug] Force refresh requested, skipping cache`
      );
    }

    // Rate limit the request
    console.log(
      `[Katana Token Debug] Entering rate limiter for token: ${normalizedAddress}`
    );
    await sushiLimiter.schedule(async () => {
      try {
        console.log(
          `[Katana Token Debug] Rate limiter executed, fetching token data for: ${normalizedAddress}`
        );
        ...
        console.log(
          `[Katana Token Debug] Fetching token info with query:`,
          tokenQuery
        );
        console.log(`[Katana Token Debug] Token variables:`, tokenVariables);
        console.log(
          `[Katana Token Debug] Using Katana subgraph URL: ${KATANA_SUBGRAPH_URL}`
        );
        ...
        console.log(
          `[Katana Token Debug] Token response:`,
        ...
        if (
          ...
          console.log(
            `[Katana Token Debug] Token not found: ${normalizedAddress} on Katana`
          );
          res.status(404).json({
            status: "error",
            msg: "Token not found on Katana",
            tokenAddress: normalizedAddress,
            debug: {
        ...
        console.log(`[Katana Token Debug] Token found:`, tokenInfo);
        ...
        console.log(
          `[Katana Token Debug] Fetching token OHLC data with variables:`,
          ohlcVariables
        );
        ...
        console.log(
          `[Katana Token Debug] Token OHLC response status: ${ohlcResponse.status}`
        );
        console.log(
          `[Katana Token Debug] Token OHLC response data:`,
        ...
        if (!rawData || rawData.length === 0) {
          console.log(
            `[Katana Token Debug] No token OHLC data found for: ${normalizedAddress}`
          );
          ...
          try {
            console.log(
              `[Katana Token Debug] Checking for any recent token data`
            );
            ...
            console.log(
              `[Katana Token Debug] Recent token data check response:`,
            ...
            if (
              ...
              console.log(
                `[Katana Token Debug] Found recent token data, using it instead`
              );
              ...
              const ohlcData: OHLCPoint[] = useableData.map((item, index) => {
                ...
                if (index < 3) {
                  console.log(
                    `[Katana Token Debug] Processed recent token item ${index}:`,
                    {
                      original: item,
              ...
              try {
                ...
                console.log(
                  `[Katana Token Debug] Successfully cached recent token OHLC data`
                );
              } catch (cacheError) {
                console.warn(
                  `[Katana Token Debug] Cache write error:`,
                  cacheError
                );
              }

              console.log(
                `[Katana Token Debug] Sending successful response with recent token data`
              );
              res.status(200).json({
                status: "success",
                data: responseData,
            ...
            console.error(
              `[Katana Token Debug] Recent token data check failed:`,
              recentDataError
            );
          }
          ...
          res.status(404).json({
            status: "error",
            msg: "No token OHLC data available for this token on Katana",
            tokenAddress: normalizedAddress,
            debug: {
              timeframe,
              startTime,
        ...
        console.log(
          `[Katana Token Debug] Processing ${rawData.length} token OHLC points`
        );
        ...
        const ohlcData: OHLCPoint[] = rawData.map((item, index) => {
          ...
          if (index < 3) {
            console.log(`[Katana Token Debug] Processed token item ${index}:`, {
              original: item,
              processed: processedItem,
            });
          }
        ...
        console.log(
          `[Katana Token Debug] Successfully processed ${ohlcData.length} token OHLC points`
        );
        ...
        console.log(`[Katana Token Debug] Response data prepared:`, {
        ...
        try {
          console.log(
            `[Katana Token Debug] Caching data with key: ${cacheKey}, TTL: ${TOKEN_OHLC_CACHE_TTL}`
          );
          await storeValue(
            cacheKey,
            JSON.stringify(responseData),
          ...
          console.log(
            `[Katana Token Debug] Successfully cached token OHLC data for ${normalizedAddress}`
          );
        } catch (cacheError) {
          console.warn(
            `[Katana Token Debug] Cache write error for ${cacheKey}:`,
            cacheError
          );
        }

        console.log(`[Katana Token Debug] Sending successful response`);
        res.status(200).json({
          status: "success",
          data: responseData,
          source: "katana-sushiswap",
        ...
        console.error(
          `[Katana Token Debug] API error for ${normalizedAddress}:`,
          {
        ...
        if (apiError.response?.status === 404) {
          res.status(404).json({
            status: "error",
            msg: "Token not found or no trading data available on Katana",
            tokenAddress: normalizedAddress,
          });
          ...
          res.status(500).json({
            status: "error",
            msg: "Failed to fetch token OHLC data from Katana",
            debug: {
    ...
    console.error("[Katana Token Debug] OHLC controller error:", {
 ...
 */
export async function clearKatanaTokenOHLCCache(
  ...
  try {
    ...
    console.log(
      `[Katana Token Debug] Clear cache request for: ${tokenAddress}`
    );
    ...
    const normalizedAddress = tokenAddress.toLowerCase();
    console.log(
      `[Katana Token Debug] Clearing cache for: ${normalizedAddress}`
    );
    ...
    console.error("[Katana Token Debug] Clear cache error:", error);
    res.status(500).json({
      status: "error",
      msg: "Unexpected server error",
    });
